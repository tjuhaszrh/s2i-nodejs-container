NodeJS {{ spec.version }} minimal container image
=========================

This container image includes Node.JS {{ spec.version }} on top of a minimal base image for your Node.JS {{ spec.version }} applications. This image is designed to be used
with the full-sized s2i-enabled Node.JS {{ spec.version }} image to build the application. The image can be used as a standalone s2i-enabled image as well,
but compared to the full-sized Node.JS {{ spec.version }} image it will be missing many build-time dependencies.
...
Node.js {{ spec.version }} available as a minimal container is a base platform for
running various Node.js {{ spec.version }} applications and frameworks.
...
Usage in OpenShift
------------------
In this example, we will assume that you are using the `ubi8/nodejs-{{ spec.version }}` image, available via `nodejs:{{ spec.version }}-ubi8` imagestream tag in Openshift
to build the application, as well as the `ubi8/nodejs-{{ spec.version }}-minimal` image, available via `nodejs:{{ spec.version }}-ubi8-minimal` image stream
...
spec:
  ...
  source:
    dockerfile: |-
      FROM nodejs:{{ spec.version }}-ubi8-minimal
      COPY src $HOME
  ...
  strategy:
    dockerStrategy:
      from:
        kind: ImageStreamTag
        name: nodejs:{{ spec.version }}-ubi8-minimal
  triggers:
  - imageChange: {}
    type: ImageChange
```
...
```
podman pull ubi8/nodejs-{{ spec.version }}
podman pull ubi8/nodejs-{{ spec.version }}-minimal
```

The UBI images `ubi8/nodejs-{{ spec.version }}` and `ubi8/nodejs-{{ spec.version }}-minimal` that are used in this example are both usable and freely redistributable under the terms of the UBI End User License Agreement (EULA). See more about UBI at [UBI FAQ](https://developers.redhat.com/articles/ubi-faq).
...
FROM ubi8/nodejs-{{ spec.version }} as builder
...
FROM ubi8/nodejs-{{ spec.version }}-minimal
