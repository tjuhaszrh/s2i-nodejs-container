NodeJS {{ spec.version }} container image
=========================

This container image includes Node.JS {{ spec.version }} as a [S2I](https://github.com/openshift/source-to-image) base image for your Node.JS {{ spec.version }} applications.
...
Node.js {{ spec.version }} available as container is a base platform for 
building and running various Node.js {{ spec.version }} applications and frameworks. 
...
Usage in OpenShift
------------------
In this example, we will assume that you are using the `ubi8/nodejs-{{ spec.version }}` image, available via `nodejs:{{ spec.version }}` imagestream tag in Openshift.

To build a simple [nodejs-sample-app](https://github.com/sclorg/nodejs-ex.git) application in Openshift:

```
oc new-app nodejs:{{ spec.version }}~https://github.com/sclorg/nodejs-ex.git
```
...
An UBI image `ubi8/nodejs-{{ spec.version }}` is used in this example. This image is usable and freely redistributable under the terms of the UBI End User License Agreement (EULA). See more about UBI at [UBI FAQ](https://developers.redhat.com/articles/ubi-faq).
